...
_start_local: _ensure_deps _check_ports
	...
	@PYTHONPATH=backend DATABASE_URL=postgresql://quant:quant123@localhost:5432/quant \
		REDIS_URL=redis://localhost:6379 LOG_LEVEL=$(LOG_LEVEL) LOG_FORMAT=$(LOG_FORMAT) \
		nohup uv run python -m domains.stock_hub.api.mcp.server \
		> $(PID_DIR)/mcp-stock.log 2>&1 & echo $$! > $(PID_DIR)/mcp-stock.pid
	@sleep 1
	@echo "[4/6] 启动前端..."
...
_stop_local:
	...
	@-pkill -TERM -f "domains.stock_hub.api.mcp.server" 2>/dev/null || true
	...
	@-pkill -9 -f "domains.stock_hub.api.mcp.server" 2>/dev/null || true
...
status:
	...
	@if pgrep -f "domains.stock_hub.api.mcp.server" > /dev/null 2>&1; then echo "  MCP Stock:      运行中"; else echo "  MCP Stock:      未运行"; fi
