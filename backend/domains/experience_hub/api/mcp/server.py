...
class ExperienceHubMCPServer(BaseMCPServer):
    """
    经验知识库 MCP 服务器

    继承 mcp_core.BaseMCPServer，注册经验相关的工具。
    """
...
def create_experience_hub_config(
    host: str = "0.0.0.0",
    port: int = 6794,
    log_level: str = "INFO",
...
def create_mcp_server(config: Optional[MCPConfig] = None):
    ...
    """
    if config is None:
        config = create_experience_hub_config()

    server = ExperienceHubMCPServer(config)
